...
def select_file(root, blender_path):
    ...
    if file_path:
        ...
        if output_directory:
            root.destroy()
            run_blender_instances(file_path, blender_path, output_directory)
...
def run_blender_instances(blend_file, blender_path, output_directory):
    frame_start = 1
    frame_end = 250
    
    for i in range(render_interval):
        frame_step = render_interval
        start_frame = frame_start + i
        output_path = os.path.join(output_directory, f"frame_####_{i}")
        command = [
            blender_path, 
            "-b", blend_file,
            "-s", str(start_frame),
    ...
    monitor_output(output_directory)

def monitor_output(output_directory):
    # Create a simple GUI to display the most recent frame
    root = tk.Tk()
    root.title("Rendering Monitor")
    label = Label(root, text="Most Recent Rendered Frame:")
    label.pack(pady=20)
    img_label = Label(root)
    img_label.pack()
    root.after(1000, lambda: update_image(img_label, output_directory))
    root.mainloop()
...
def update_image(img_label, output_directory):
    ...
    try:
        ...
        photo = tk.PhotoImage(file=latest_file)
